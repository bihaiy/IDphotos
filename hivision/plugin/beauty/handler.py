...
def beauty_face(ctx: Context):
    ...
    if ctx.params.whitening_strength > 0:
        print(f"正在进行美白处理，强度: {ctx.params.whitening_strength}")
    ...
    if (
        ...
        print(f"美颜参数: 亮度={ctx.params.brightness_strength}, "
              f"对比度={ctx.params.contrast_strength}, "
              f"锐化={ctx.params.sharpen_strength}, "
              f"饱和度={ctx.params.saturation_strength}")
